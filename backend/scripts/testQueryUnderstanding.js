...

{
  ...
  "functional_signals": string[], // AT MOST 5 capability/architecture-level tags describing what KIND of system this is — drawn only from what's explicitly described, never invented. These must be REUSABLE CATEGORY tags in the same style as a project tagging system (e.g. "multi-tenant", "offline-sync", "real-time", "webhooks", "api-integration", "high-reliability", "biometric-auth", "payment-processing", "voice-assistant", "compliance", "field-ops", "lead-generation"), NOT a paraphrase of each bullet point in the JD. Do NOT decompose a feature list into one tag per feature (e.g. a JD listing "student dashboard, course listing, progress tracking, quiz, admin panel" should NOT produce 5 separate signals like "student-dashboard"/"quiz-functionality" — those are one-off feature names specific to this JD that will never match anything else. Instead ask: "what reusable architectural pattern do these features together represent?" (e.g. that whole feature list is just "admin-panel" + "progress-tracking" as the two patterns worth naming, or even just "content-management" if nothing more specific fits). If nothing rises above generic CRUD/dashboard functionality, return an empty array rather than forcing tags.
